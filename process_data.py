...
def preprocess_by_word(data):
    stops_file = open("stop_words.txt",
                      "r")  # each (unique) stop word is on its own line, with extra white space for padding
    stop_words = set([line.strip() for line in stops_file if
    ...
    for document in data:
        ...
        for word in document:
            if word not in stop_words:  # only count words not in stop words
                nopunct = word.translate(str.maketrans('', '', string.punctuation))  # remove punctuation
